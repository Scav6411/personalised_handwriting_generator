...
class SLRGAN(nn.Module):
  ...
  def __init__(self):
    ...
    self.optimizer_G = torch.optim.Adam(self.netG.parameters(),
                                        lr=0.00009, betas=(0.0, 0.999), weight_decay=0, eps=1e-10)
    self.optimizer_OCR = torch.optim.Adam(self.netOCR.parameters(),
                                          lr=0.00009, betas=(0.0, 0.999), weight_decay=0,
                                          eps=1e-10)
  ...
  def backward_D_OCR(self):
        ...
        pred_real = self.netD(self.real.detach())#Training the discriminator on the real images(self.real is real image in tensr format)

        pred_fake = self.netD(**{'x': self.fake.detach()})#Here the fake images are passed to discriminator model


        self.loss_Dreal, self.loss_Dfake = loss_hinge_dis(pred_fake, pred_real, self.len_text_fake.detach(), self.len_text.detach(), True)#calculating losses for real and fake predictions through hinge loss functions

        self.loss_D = self.loss_Dreal + self.loss_Dfake#Total loss of discriminator

        self.pred_real_OCR = self.netOCR(self.real.detach())#Here the real images are being passed to CRNN class to recognize the text in the images
        preds_size = torch.IntTensor([self.pred_real_OCR.size(0)] * batch_size).detach()
        loss_OCR_real = self.OCR_criterion(self.pred_real_OCR, self.text_encode.detach(), preds_size, self.len_text.detach())#This probably calculates the difference in the predictions made by CRNN and the actual images by passing the data to the CTC Loss function
        self.loss_OCR_real = torch.mean(loss_OCR_real[~torch.isnan(loss_OCR_real)])

        loss_total = self.loss_D + self.loss_OCR_real#Total Loss
        # backward
        loss_total.backward()
        #The below code is for re-initializing the gradients of the model to zero
  ...
  def backward_D_WL(self):
        ...
        self.loss_w_real = self.netW(self.real.detach(), self.input['wcl'].to(DEVICE)).mean()#Ths loss on real images for wdiscriminator is calculated??
        # total loss
